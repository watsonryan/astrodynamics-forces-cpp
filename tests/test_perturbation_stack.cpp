...
int main() {
  ...
  stack.add(std::make_unique<ConstantPerturbation>(atmo::Vec3{1.0e-9, -2.0e-9, 3.0e-9}, "bias"));
  ...
  const atmo::Vec3 expected_total =
      direct.acceleration_mps2 + atmo::Vec3{1.0e-9, -2.0e-9, 3.0e-9};
