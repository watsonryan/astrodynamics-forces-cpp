...
flowchart LR
  ...
  F --> M[antenna_thrust_cli]
  F --> N[antenna_thrust_batch_cli]
  F --> W[relativity_cli]
  F --> K[gravity_sph_cli]
  F --> S[srp_cli]
  F --> U[srp_batch_cli]
...
- `forces-core`: drag, antenna thrust, full SPH gravity+tides, Earth Radiation, SRP, relativity, third-body, and generic perturbation interfaces
...
- `apps/forces-cli/antenna_thrust_cli.cpp`: single-state antenna thrust evaluation
- `apps/forces-cli/antenna_thrust_batch_cli.cpp`: batched antenna thrust outputs
...
- `astroforces::forces::AntennaThrustPerturbationModel`: antenna thrust implementation of the generic interface.
